...
def macd(symbol, period=default_periods, backfill=False, refresh=False, start_date=config.start_date, end_date=config.end_date):
...
def plot_macd(symbol, period=default_periods, backfill=False, refresh=False, start_date=config.start_date, end_date=config.end_date):
    ...
    signal_column_name = "MACD" + str(period[0])
    if macd_column_name not in df.columns or signal_column_name not in df.columns:
        df = df.join(macd(symbol, period, backfill=backfill, refresh=False, start_date=start_date, end_date=end_date))
    # if len(df) > period[1] and len(df) > period[2]:  # to prevent AttributeError when the column is all None
