...
sp500_full_table_filename = "SP500FullTable.csv"
sp500_symbols_filename = "SP500Symbols.csv"
sp500_full_table_path = config.join(config.sp500_symbols_data_path, sp500_full_table_filename)
sp500_symbols_path = config.join(config.sp500_symbols_data_path, sp500_symbols_filename)
...
def download_sp500():
    ...
    df.to_csv(sp500_symbols_path, columns=["Symbol"], index=False)
...
def get_sp500():
    ...
    if not config.path.exists(sp500_symbols_path):
        download_sp500()
...
def get_sp500_by_sector(sector):
    ...
    if not config.path.exists(sp500_symbols_path):
        download_sp500()
    df = pd.read_csv(sp500_full_table_path, usecols=["Symbol", "GICS Sector"])
    if type(sector) is str:
        df = df.loc[df["GICS Sector"] == sector]
    elif isinstance(sector, list):
        df = df.loc[df["GICS Sector"].isin(sector)]
    return df["Symbol"].tolist()
...
def get_sp500_by_sub_sector(sector):
    ...
    if not config.path.exists(sp500_symbols_path):
        download_sp500()
    df = pd.read_csv(sp500_full_table_path, usecols=["Symbol", "GICS Sub Industry"])
    if type(sector) is str:
        df = df.loc[df["GICS Sub Industry"] == sector]
    elif isinstance(sector, list):
        df = df.loc[df["GICS Sub Industry"].isin(sector)]
    return df["Symbol"].tolist()
...
def get_all_sp500_sectors():
    ...
    if not config.path.exists(sp500_symbols_path):
        download_sp500()
    df = pd.read_csv(sp500_full_table_path, usecols=["GICS Sector"])
    return sorted(df["GICS Sector"].unique())
...
def get_all_sp500_sub_sectors():
    ...
    if not config.path.exists(sp500_symbols_path):
        download_sp500()
    df = pd.read_csv(sp500_full_table_path, usecols=["GICS Sub Industry"])
    return sorted(df["GICS Sub Industry"].unique())
