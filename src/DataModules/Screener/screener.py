    ...
    for func in [ema, sma, macd, rsi, bb]:
        ...
        for symbol in sp.get_sp500():
            ...
            if df.index[-1].date() != datetime.date.today():
                print("Last date in file does not equal todays date. Is today a weekend or a holiday?")
        ...
        df.to_csv(utils.get_file_path(config.ta_data_path, func.get_signal_name() + table_filename, dated=True, start_date="", end_date=datetime.date.today()))
